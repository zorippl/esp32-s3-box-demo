...
LV_IMG_DECLARE(note_icon)
LV_IMG_DECLARE(album_icon)
LV_IMG_DECLARE(air_icon)
    ...
    if (LV_EVENT_VALUE_CHANGED == event->code) {
        if (btn == obj_dev[0]) {
            if (btn->state & LV_STATE_CHECKED) {
                ...
                ESP_ERROR_CHECK(app_led_set_all(0, 0, 0));
            }
        }else if (btn == obj_dev[1]){
            ui_dev_ctrl(false);
            ui_note(true);
        }else if (btn == obj_dev[2]) {
            ui_dev_ctrl(false);
            ui_air_ctrl(true);
        }else if (btn == obj_dev[3]) {
            ui_dev_ctrl(false);
            ui_album(true);
        }
    }
...
void ui_dev_ctrl(bool show)
{
    ...
    static const btn_img_src_t img_src_list[] = {
        ...
        { .name = "Note", .img_on = &note_icon, .img_off = &note_icon },
        { .name = "Air", .img_on = &air_icon, .img_off = &air_icon },
        { .name = "Album", .img_on = &album_icon, .img_off = &album_icon },
    };
    /* 依次部署四个按键 */
