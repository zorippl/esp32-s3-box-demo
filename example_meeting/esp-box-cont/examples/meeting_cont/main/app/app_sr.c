...
static afe_config_t afe_config = {
    ...
    .agc_mode = AFE_MN_PEAK_AGC_MODE_3,  /* 1 : -5dB 2: -4dB ... */
...
static const esp_afe_sr_iface_t *afe_handle = &esp_afe_sr;
...
static void audio_detect_task(void *pvParam)
{
    ...
    esp_afe_sr_data_t *afe_data = (esp_afe_sr_data_t *) pvParam;
    int multinet_work = 0;
    esp_mn_state_t command_state = ESP_MN_STATE_DETECTING;
    ...
    while (true) {
        ...
        if (AFE_FETCH_CHANNEL_VERIFIED == ret_val) {
            ...
            if (multinet_work) {
                multinet->wakenet_reset(model_data);
                // printf("!!!\n");
            }
        }
        ...
        if (true == detect_flag) {
            ...
            if (false == audio_is_playing()) {
                command_state = multinet->detect(model_data, detect_buff);
            } else {
                continue;
            }

            if (ESP_MN_STATE_DETECTING == command_state) {
                sr_command_id = -1;
                continue;
            }

            if (ESP_MN_STATE_TIMEOUT == command_state) {
                sr_command_id = -2;
            ...
            if (ESP_MN_STATE_DETECTED == command_state) {
                esp_mn_results_t *mn_result = multinet->get_results(model_data);
                sr_command_id = mn_result->command_id[0];
