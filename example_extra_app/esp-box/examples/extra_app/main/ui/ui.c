...
static char *week[] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
...
static void clock_blink_cb(lv_timer_t *timer)
{
    ...
    static char date_str[24] = {0};
    snprintf(date_str, sizeof(date_str), "%s, %s %d", week[time.tm_wday], month[time.tm_mon], time.tm_mday);

    ui_clock_set_time(time_str);
    ui_clock_set_date(date_str);
