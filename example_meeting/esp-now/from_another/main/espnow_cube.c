...
uint8_t CMD_TURN_OFF[] = {0x00, 0x00, 0x00, 0x00, 0x10};
...
void turn_on_light(bool on)
{
    ...
    if(on) {
        memcpy(buf->payload, CMD_TURN_ON, CONFIG_ESPNOW_SEND_LEN - sizeof(example_espnow_data_t));
    } else {
        memcpy(buf->payload, CMD_TURN_OFF, CONFIG_ESPNOW_SEND_LEN - sizeof(example_espnow_data_t));
    }
    ...
    for(int i = 0; i < ESP_NOW_ETH_ALEN; i++) {
        printf("%02x ", send_param->dest_mac[i]);
    }
    printf("\n");
    ...
    while (xQueueReceive(s_example_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
        switch (evt.id) {
            case EXAMPLE_ESPNOW_SEND_CB:
            {
                ...
                ESP_LOGE(TAG, "not broadcast message");
                break;
